            ...
            <div class="query-list-compact">
              ...
              <div v-for="(q, index) in sqlAnalysis.slowestQueries.slice(0, 3)" :key="index" class="query-item-compact">
                ...
                <button
                  ...
                  @click="runExplain(q.query, q.variables, { table: q.table, operation: q.operation, type: 'slowest' })"
                >
                  EXPLAIN
                </button>
              </div>
          ...
          <div class="analyzer-subsection">
            ...
            <div class="query-list-compact">
              ...
              >
                ...
                <button
                  class="btn-explain-compact"
                  @click="runExplain(item.example.query, item.example.variables, { table: item.example.table, operation: item.example.operation, type: 'frequent' })"
                >
                  EXPLAIN
                </button>
              </div>
          ...
          >
            <div class="request-header-line">
              ...
              <span
                ...
                :class="{ 'status-success': req.statusCode >= 200 && req.statusCode < 300, 'status-error': req.statusCode >= 400 }"
                >{{ req.statusCode }}</span
              >
            </div>
    ...
    <div v-if="selectedLog" class="modal-body">
      <div class="modal-section">
        ...
        <pre
          ...
          v-html="convertAnsiToHtml(selectedLog.entry?.raw.replaceAll('\\n', '\n').replaceAll('\\t', '    ') || 'No raw log available')"
        ></pre>
      </div>
      <div class="modal-section">
        <h4>Parsed Fields</h4>
