...
watch(() => props.requestIdFilter, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    sendFilterUpdate()
  }
})

watch(() => props.containerFilter, () => {
  sendFilterUpdate()
}, { deep: true })
...
function sendFilterUpdate() {
  ...
  const filter = {
    selectedContainers:
      props.containerFilter.length > 0 ? props.containerFilter : containers.value.map((c) => c.Name),
